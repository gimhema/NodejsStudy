...
struct UNREALCLIENT_API FUDPSettings
{
...
class UNREALCLIENT_API FUDPNative
{
public:
...
class UNREALCLIENT_API UUDPComponent : public UActorComponent
{
...
public:
	...
	UFUNCTION(BlueprintCallable, Category = "UDP Functions")
	void FStringToTArray(const FString& InputString, TArray<uint8>& OutArray);

	UFUNCTION(BlueprintCallable, Category = "UDP Functions")
	bool EmitString(const FString& InputString);
		
	UFUNCTION(BlueprintCallable, Category = "UDP Functions")
	bool EmitJson(const TSharedPtr<FJsonObject>& JsonObject);

	void JsonToTBytes(const TSharedPtr<FJsonObject>& JsonObject, TArray<uint8>& OutArray);

	TSharedPtr<FJsonObject> BytesToJson(const TArray<uint8>& InArray);

	FString JsonToFString(const TSharedPtr<FJsonObject>& JsonObject);
};
